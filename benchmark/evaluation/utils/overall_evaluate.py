...
def evaluate(dataset, output_path, groundtruth, parsedresult, result_path):
    ...
    GA, FGA = get_group_accuracy(df_groundtruth['EventTemplate'],
                                 df_parsedlog['EventTemplate'])

    correctly_parsed_messages = df_parsedlog[['EventTemplate']].eq(
        df_groundtruth[['EventTemplate']]).values.sum()
...
def get_group_accuracy(series_groundtruth, series_parsedlog, debug=False):
    ...
    for parsed_eventId in series_parsedlog_valuecounts.index:
        ...
        series_groundtruth_logId_valuecounts = series_groundtruth[
            logIds].value_counts()
        error_eventIds = (parsed_eventId,
...
def find_corr_oracle_templates(log_message_ids, groundtruth_df):
    ...
    corresponding_oracle_templates = groundtruth_df.merge(log_message_ids,
                                                          on='LineId')
    corresponding_oracle_templates = list(
        corresponding_oracle_templates.EventTemplate.unique())
...
def evaluate_template_level(dataset, groundtruth, parsedresult, output_dir):
    ...
    oracle_templates = list(
        pd.read_csv(groundtruth)['EventTemplate'].drop_duplicates().dropna())
    identified_templates = list(
        pd.read_csv(parsedresult)['EventTemplate'].drop_duplicates().dropna())
    ...
    for identified_template in identified_templates:
        ...
        log_message_ids = parsedresult_df.loc[
            parsedresult_df['EventTemplate'] == identified_template, 'LineId']
        ...
        corr_oracle_templates = find_corr_oracle_templates(
            log_message_ids, groundtruth_df)
...
def compute_template_level_accuracy(num_oracle_template,
    ...
    precision = len(comparison_results_df[comparison_results_df.type ==
                                          'SM']) / count_total  # PTA
    recall = len(
        comparison_results_df[comparison_results_df.type == 'SM']) / float(
            num_oracle_template)  # RTA
    over_generalized = len(comparison_results_df[comparison_results_df.type ==
                                                 'OG']) / count_total
    under_generalized = len(comparison_results_df[comparison_results_df.type ==
                                                  'UG']) / count_total
    mixed = len(comparison_results_df[comparison_results_df.type ==
                                      'MX']) / count_total
    f1_measure = 0.0
...
def evaluate_template_level_all(dataset, groundtruth, parsedresult,
                                output_dir):
    """
    Conduct the template-level analysis using 4-type classifications
    ...
    for identified_template in identified_templates:
        ...
        comparison_results.append([
            identified_template, identified_template_type,
            corr_oracle_templates, num_messages
        ])

    comparison_results_df = pd.DataFrame(comparison_results,
                                         columns=[
                                             'identified_template', 'type',
                                             'corr_oracle_templates',
                                             'num_messages'
                                         ])
    comparison_results_df.to_csv(os.path.join(
        output_dir, dataset + '_template_analysis_results.csv'),
                                 index=False)
    (F1_measure, PTA, RTA, OG, UG,
     MX) = compute_template_level_accuracy(len(oracle_templates),
                                           comparison_results_df)
    print(
        'F1: {:.4f}, PTA: {:.4f}, RTA: {:.4f}, OG: {:.4f}, UG: {:.4f}, MX: {:.4f}'
        .format(F1_measure, PTA, RTA, OG, UG, MX))
    return len(identified_templates), len(
        oracle_templates), F1_measure, PTA, RTA, OG, UG, MX
