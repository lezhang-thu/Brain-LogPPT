...
def train():
    ...
    progress_bar = tqdm(range(train_args.max_train_steps),
    ...
    best_metric = evaluate(metric, model, tokenizer, eval_dataloader,
                           accelerator, main_args.pad_to_max_length,
                           label_token_id_to_label, id_to_label,
                           main_args.mode)
    ...
    if main_args.output_dir is not None:
        ...
        unwrapped_model.save_pretrained(main_args.output_dir,
...
if __name__ == '__main__':
    ...
    if task_args.task_name == "log-parsing":
        raw_datasets, text_column_name, label_name = load_data_parsing(
            main_args)
    else:
        raw_datasets, text_column_name, label_name = load_data_anomaly_detection(
            main_args)
    ...
    sorted_add_tokens = sorted(list(ori_label_token_map.keys()),
                               key=lambda x: len(x),
                               reverse=True)
    ...
    label_token_to_id = {
        label: tokenizer.convert_tokens_to_ids(label_token)
        for label, label_token in label_token_map.items()
    }
    ...
    if task_args.task_name == "log-parsing":
        processed_raw_datasets, label_words, keywords = parsing_tokenize_dataset(
            tokenizer, raw_datasets, text_column_name, label_name,
            main_args.max_length, padding, label_to_id, label_token_to_id,
            model_type, main_args.mode)
    ...
    if main_args.pad_to_max_length:
        ...
        data_collator = CustomDataCollator(tokenizer, pad_to_multiple_of=None)
    ...
    if main_args.mode == "prompt-tuning":
        label_words = find_labels(model, train_dataloader, eval_dataloader,
                                  keywords)
        selected_words = []
        ...
        for k in lbl_word_indices:
            token = tokenizer.convert_tokens_to_string(
                [tokenizer.convert_ids_to_tokens(k)]).strip()
        ...
        label_words = {
            'i-val': list(label_words.keys())[:train_args.no_label_words]
        }
        print(tokenizer.convert_ids_to_tokens(label_words['i-val']))
        model = add_label_token(model_type, model, tokenizer, label_words,
                                train_args.wo_label_words)
    ...
    label_id_list = torch.tensor([
        label_token_to_id[id_to_label[i]] for i in range(len(id_to_label))
        if i != 0 and not id_to_label[i].startswith("B-")
    ],
                                 dtype=torch.long,
                                 device=device)
