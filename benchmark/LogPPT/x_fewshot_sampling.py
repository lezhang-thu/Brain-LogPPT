...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        os.makedirs("datasets-wo-virtual-label/{0}".format(dataset),
                    exist_ok=True)

        logdf = log_to_dataframe(f'../../2k_dataset/{setting["log_file"]}',
                                 setting['log_format'])
        logdf.to_csv(
            f"datasets-wo-virtual-label/{setting['log_file']}_structured.csv")

        labelled_logs = pd.read_csv(
            f'../../2k_dataset/{setting["log_file"]}_structured_corrected.csv')
        test_samples = [(row['Content'], row['EventTemplate'])
                        for _, row in labelled_logs.iterrows()]
        ...
        content = [(clean(x), i, len(x))
                   for i, x in enumerate(logdf['Content'].tolist())]
        content = [x for x in content if len(x[0].split()) > 1]
        content = shuffle(content)

        for shot in [32]:
            keywords_list = []
            os.makedirs("datasets-wo-virtual-label/{0}/{1}shot".format(
                dataset, shot),
                        exist_ok=True)
            samples_ids = adaptive_random_sampling(shuffle(content), shot)

            labeled_samples = [
                (row['Content'], template_dict[row['Content']])
                for _, row in logdf.take(samples_ids).iterrows()
            ]
            labeled_samples = [{
                "text": x[0],
                "label": x[1],
            ...
            with open(
                    "datasets-wo-virtual-label/{0}/{1}shot/{2}.json".format(
                        dataset, shot, 1), "w") as f:
