    ...
    try:
        GA, PA, ED, _, _, _, _, _, _, unseen_PA, no_unseen = evaluator(
            groundtruth=os.path.join(indir,
                                     log_file + '_structured_corrected.csv'),
            parsedresult=os.path.join(output_dir,
                                      log_file + '_structured.csv'))
        bechmark_result.append([dataset, GA, PA, ED, unseen_PA,
                                no_unseen])  # , _, _, _, _, _, _])
        avg_ga += GA
        avg_pa += PA
        avg_ed += ED
...
bechmark_result.append([
    "Average", avg_ga / len(benchmark_settings.keys()),
    avg_pa / len(benchmark_settings.keys()),
...
df_result = pd.DataFrame(bechmark_result,
                         columns=[
                             'Dataset', 'Group Accuracy', 'Parsing Accuracy',
                             'Edit distance', 'unseen_PA', 'no_unseen'
                         ])
