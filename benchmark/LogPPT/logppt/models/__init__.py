...
def add_label_token(lm_name,
                    model,
                    tokenizer,
                    label_map,
                    wo_label_words=False):
    if "roberta" in lm_name:
        return add_label_token_roberta(model, tokenizer, label_map,
                                       wo_label_words)
    elif "bert" in lm_name:
        return add_label_token_bert(model, tokenizer, label_map,
                                    wo_label_words)
    elif "gpt2" in lm_name:
        return add_label_token_gpt2(model, tokenizer, label_map,
                                    wo_label_words)
    elif "xlnet" in lm_name:
        return add_label_token_xlnet(model, tokenizer, label_map,
                                     wo_label_words)
    else:
        raise NotImplementedError("Not implemented yet")
