...
def get_labels(predictions,
    ...
    if mode == "prompt-tuning":
        true_predictions = [[
            label_token_id_to_label[p].upper()
            if p in label_token_id_to_label.keys() else "O"
            for (p, l) in zip(pred, gold_label) if l != -100
        ] for pred, gold_label in zip(y_pred, y_true)]
    else:
        true_predictions = [[
            id_to_label[p].upper() for (p, l) in zip(pred, gold_label)
            if l != -100
        ] for pred, gold_label in zip(y_pred, y_true)]
    ...
    ori_tokens = [[
        ...
        for (p, l, t) in zip(pred, gold_label, token) if l != -100
    ] for pred, gold_label, token in zip(y_pred, y_true, x_tokens)]

    true_labels = [[
        'I-{}'.format(l[2:].upper()) if l != "O" else "O" for l in label
...
def evaluate(metric,
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        if not pad_to_max_length:  # necessary to pad predictions and labels for being gathered
            ...
            labels = accelerator.pad_across_processes(labels,
                                                      dim=1,
                                                      pad_index=-100)
            token_labels = accelerator.pad_across_processes(token_labels,
                                                            dim=1,
                                                            pad_index=-100)
        predictions_gathered = accelerator.gather(predictions)
        labels_gathered = accelerator.gather(labels)
        token_labels_gathered = accelerator.gather(token_labels)
        preds, refs, tokens = get_labels(predictions_gathered, labels_gathered,
                                         token_labels_gathered, device,
    ...
    label = "overall"
    print("{}: {}, {}: {}, {}: {}, {}: {}".format(
        label + "_precision", eval_metric[label + "_precision"],
        label + "_recall", eval_metric[label + "_recall"], label + "_f1",
        eval_metric[label + "_f1"], label + "_accuracy",
