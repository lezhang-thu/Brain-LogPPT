...
def prepare_results(output_dir, otc):
    ...
    with open(os.path.join(output_dir, result_file), 'w') as csv_file:
        ...
        fw.writerow([
            'Dataset', 'GA_time', 'PA_time', 'TA_time', 'parse_time',
            'identified_templates', 'ground_templates', 'GA', 'PA', 'FTA',
            'PTA', 'RTA', 'OG', 'UG', 'MX'
        ])
...
def evaluator(groundtruth, parsedresult):
    ...
    GA, PA, ED, _, unseen_PA, no_unseen = evaluate(groundtruth=groundtruth,
                                                   parsedresult=parsedresult)
    GA_end_time = time.time() - start_time
    print(
        'Grouping and Parsing Accuracy calculation done. [Time taken: {:.3f}]'.
        format(GA_end_time))
