...
def map_template_xlnet(tokenizer, c, t, mode="prompt-tuning"):
    val_token = tokenizer.convert_tokens_to_ids(
        'i-val') if mode == "prompt-tuning" else 1
...
def template_extraction(tokenizer,
                        model,
                        accelerator,
                        log_file,
                        max_length,
    ...
    def tokenize_and_align_labels(examples):
        examples['Content'] = [
            " ".join(x.split()) for x in examples['Content']
        ]
    ...
    if 'roberta' in model_name:
        event_templates = [(map_template_roberta(tokenizer,
                                                 x[0],
                                                 x[1],
                                                 mode=mode), x[2])
        ...
        event_templates = [
            map_template_bert(tokenizer, x[0], x[1], content[i], mode=mode)
            for i, x in enumerate(events)
        ]
        event_list = [x.strip() for x in event_templates]
    elif 'xlnet' in model_name:
        event_templates = [(map_template_xlnet(tokenizer,
                                               x[0],
                                               x[1],
                                               mode=mode), x[2])
                           for i, x in enumerate(events)]
        ...
        event_templates = [(map_template_gpt2(tokenizer, x[0], x[1],
                                              mode=mode), x[2])
                           for i, x in enumerate(events)]
        event_list = [""] * len(test_dataset)
        for (e, idx) in event_templates:
    ...
    if len(df) <= 2000:
        ...
        df.to_csv(
            os.path.join(o_dir, dataset_name + "_full.log_structured.csv"))
