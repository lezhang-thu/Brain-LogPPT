...
def modify_prompt(new_prompt, dataset):
    new_df = pd.read_csv(
        f"../../full_dataset/{dataset}/{dataset}_full.log_structured.csv")
    new_df = new_df.drop_duplicates(subset=['Content'])
    print(dataset, len(new_prompt))
    logs = [
        re.sub(' +', ' ', new_prompt[i]['text']).strip()
        for i in range(len(new_prompt))
    ]
    new_df = new_df[new_df['Content'].isin(logs)]
    for i in range(len(new_prompt)):
        new_prompt[i]['text'] = re.sub(' +', ' ',
                                       new_prompt[i]['text']).strip()
        if len(new_df[new_df['Content'] == new_prompt[i]['text']]) > 0:
            new_prompt[i]['label'] = new_df[new_df['Content'] == new_prompt[i]
                                            ['text']].iloc[0]['EventTemplate']
    return new_prompt
