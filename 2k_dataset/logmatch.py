...
benchmark_settings = {
    ...
    'Proxifier': {
        ...
        'regex': [
            r'<\d+\ssec', r'([\w-]+\.)+[\w-]+(:\d+)?', r'\d{2}:\d{2}(:\d{2})*',
            r'[KGTM]B'
        ],
        'st':
        0.6,
...
def process_csv(input_file, output_file, encoding="utf-8"):
    ...
    new_df['EventTemplate'] = new_df['EventTemplate'].apply(
...
def clean_template_file(file_path):
    ...
    df = df[df["EventId"].notna() & (df["EventId"] != 'NONE')]
    df = df[df["EventTemplate"].notna() & (df["EventTemplate"] != 'NONE')]
    df = df[df["Occurrences"].notna() & (df["Occurrences"] != 'NONE') &
            (df["Occurrences"] > 0)]
    df.to_csv(output_path, index=None, encoding="utf-8")
    print("Processed length: ", len(df))
    print("===============================================================")
...
def clean_log_file(csv_path, log_path):
    ...
    new_log_path = os.path.join("../../full_dataset/",
                                log_path.replace(".log", "_full.log"))
    print(new_log_path)
    cnt = 0
...
def diff_templates(final_templates, original_templates):
    ...
    diff_templates_path = original_templates.replace("original.csv",
                                                     "diff.csv")
...
def process_full_log(dataset):
    ...
    process_csv(f"{dataset}/{dataset}.log_templates_label.csv",
                f"{dataset}/{dataset}.log_templates_original.csv",
                result['encoding'])
    output_dir = f"logmatch_result/{dataset}"  # The result directory
    full_output_dir = f"../../full_dataset/{dataset}"
    if not os.path.exists(full_output_dir):
        os.makedirs(full_output_dir)
    log_filepath = benchmark_settings[dataset][
        'log_file']  # The input log file path
    ...
    matcher = regexmatch.PatternMatch(outdir=output_dir,
                                      n_workers=n_workers,
    ...
    output_template = os.path.join("logmatch_result/",
                                   log_filepath + "_templates.csv")
    ...
    full_matcher = regexmatch.PatternMatch(outdir=full_output_dir,
                                           n_workers=n_workers,
                                           logformat=log_format,
                                           optimized=True)
    ...
    df = df.drop(["ParameterList"], axis=1)
    df.to_csv(full_log_path + "_structured_all.csv",
              index=None,
              encoding="utf-8")
